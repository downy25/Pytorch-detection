...
def get_dataset(is_train, args):
    image_root='./Robot_train' if is_train else './Robot_val'
    image_set = "train" if is_train else "val"
    num_classes, mode = {"coco": (2, "instances"), "coco_kp": (2, "person_keypoints")}[args.dataset]
    with_masks = "mask" in args.model
    ds = data.RobotSizeDataset(root=image_root,
                               transforms=get_transform(is_train=True,args=args))
    return ds, num_classes

def get_val_dataset(is_train, args):
    image_root='./Robot_val/Annotation'
    image_set = "train" if is_train else "val"
    num_classes, mode = {"coco": (2, "instances"), "coco_kp": (2, "person_keypoints")}[args.dataset]
    with_masks = "mask" in args.model
    # ds = get_coco(
...
def main(args):
    ...
    dataset_test, _ = get_val_dataset(is_train=False, args=args)
